...
class PedGenModel(LightningModule):
    ...
    def get_condition(self, batch):
        ...
        if self.use_image:
            ...
            img_feature = img[..., :-2]
            img_pos = img[..., -2:]
            img_pos_embed = positional_encoding_2d(img_pos, #来自rot.py
    ...
    def sample_ddim_progressive(self,
        ...
        with torch.no_grad():
            for i in reversed(range(0, self.ddim_timesteps)):
                ...
                if hand_shake:#当生成超长序列时，模型会分段生成，然后通过这个函数将前一段的末尾和后一段的开头进行平滑融合。
    ...
    def hand_shake(self, x0):
        ...
        return x0
    # 对生成的原始序列利用离散余弦变换 (DCT) 转换到频域，截断高频信号
    # 再通过逆变换 (IDCT) 转回时域，彻底消除行人的高频抖动
