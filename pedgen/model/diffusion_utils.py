...
#离散余弦变换，将信号从时域转换到频域
#先学习低频的大动作，再补充高频的小细节，这能有效减少动作的抖动
...
#根据当前的去噪阶段，这个模块会自动调节神经网络各层信号的强弱
#通过Time Embedding计算出Scale (缩放)和Shift (平移)
class StylizationBlock(nn.Module):

    def __init__(self, latent_dim, time_embed_dim, dropout):
        super().__init__()
        self.emb_layers = nn.Sequential(#将以下多个层组合成一个顺序执行的层列表
            nn.SiLU(),
            nn.Linear(time_embed_dim, 2 * latent_dim),
        )#将输入的时间嵌入特征经过Sigmoid激活函数后，再通过一个线性变换映射到2 *空间
        self.norm = nn.LayerNorm(latent_dim)#对每层的输出进行标准化处理
        self.out_layers = nn.Sequential(
            nn.SiLU(),
            nn.Dropout(p=dropout),#以概率dropout随机地将一部分神经元的输出设置为零，防止模型过拟合
            zero_module(nn.Linear(latent_dim, latent_dim)),#经过线性层的权重和偏置，输出的维度与输入相同
        )
    ...
    def forward(self, h, emb):
        ...
        emb: B, D
        """
        # B, 1, 2D   #unsqueeze(1)在第二个维度插入一个新的维度
        emb_out = self.emb_layers(emb).unsqueeze(1)
        # scale: B, 1, D / shift: B, 1, D
        scale, shift = torch.chunk(emb_out, 2, dim=2)#将emb_out切分为缩放和偏移两个部分
        h = self.norm(h) * (1 + scale) + shift#FilM机制，根据输入的emb特征来动态调节h
        h = self.out_layers(h)
        return h
...
class MotionTransformer(nn.Module):
    ...
    def __init__(
            ...
            latent_dim=512,
            ff_size=1024,
            num_layers=8,#Transformer层的数量
            num_heads=8,
            dropout=0.2,
            activation="gelu",
            trans_rot_sep=True,#位移与旋转分离
        ...
        if self.trans_rot_sep:
            ...
            # Input Embedding
            self.trans_embed = nn.Linear(3, self.latent_dim)#将输入的前三个特征进行映射
            self.rot_embed = nn.Linear(self.input_feats - 3, self.latent_dim)#将其余特征进行映射
        else:
            self.joint_embed = nn.Linear(self.input_feats, self.latent_dim)##将所有特征一起映射
        ...
        self.temporal_decoder_blocks = nn.ModuleList()#由多个TemporalDiffusionTransformerDecoderLayer组成的模块列表
        for i in range(num_layers):#用多层解码器进行特征提取
        ...
        # Output Module
        if self.trans_rot_sep:
            self.trans_out = zero_module(nn.Linear(self.latent_dim, 3))#位移输出层将latent_dim映射到3维
            self.rot_out = zero_module(
                nn.Linear(self.latent_dim, self.input_feats - 3))#旋转输出层将latent_dim映射到input_feats - 3维
        else:
            self.out = zero_module(nn.Linear(self.latent_dim, self.input_feats))#将整个latent_dim映射回输入特征空间
    ...
    def forward(self, x, timesteps, cond_embed=None):
        ...
        # B, T, latent_dim
        if self.trans_rot_sep:#分别处理后，再拼接，并加上序列嵌入和位移旋转嵌入
        ...
        i = 0
        prelist = []#用于存储前半层的结果，并与后半层相加（类似U-Net结构，保留动作的原始空间特征）
